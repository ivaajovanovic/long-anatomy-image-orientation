...
def read_raw(im_fname,crop_flag=0, preview_flag=0):
    ...
    L_hdr = 0
    if ext in ['.fxd','.FXD','.raw','.RAW','.vxr','.VXR','']:
        ...
        elif l == 18875392:  # Varex sa 1024 bajta zaglavlja
            im_size = [3072, 3072]
            L_hdr = 1024 # Preskačemo prvih 1024 bajta
            # Mode informacije
            DetInfo = {'Index': 26, 'Name': 'Varex4343_WithHeader', 'Type': 'DR', 'LinMax': 60000, 'PixelSize': 0.139}
        elif l == 2098176:  # Novi detektor/mod (1024x1024 sa 1024px headerom)
            im_size = [1024, 1024]
            L_hdr = 1024
            DetInfo = {'Index': 105, 'Name': 'Varex_1024_Binned', 'Type': 'DR', 'LinMax': 65535, 'PixelSize': 0.280}
...
def disp_im(im, title='', lmin=0, lmax=255):
    # Prosleđujemo title direktno u figure()
    fig = plt.figure(num=title) 
    ax = fig.add_axes([0,0,1,1])
    ax.imshow(im, cmap='gray', vmin=lmin, vmax=lmax)
    plt.axis('off')
    
    # Ako želiš da budeš sigurna da radi na svim verzijama, 
    # možeš koristiti try-except ili samo obrisati spornu liniju
    try:
        fig.canvas.manager.set_window_title(title)
    except AttributeError:
        # Za veoma stare verzije Matplotlib-a
        pass
